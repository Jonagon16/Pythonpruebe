...
def mostrar_alumnos_dataframe(dic):
    diccionario = {}
    for alumno_id, alumno in dic.items():
        diccionario[alumno_id] = {
            "Fecha": alumno.fecha,
    ...
    df = df.transpose()
    tabla_Alumnos = tabulate(df, headers='keys', tablefmt='fancy_grid')
    print(tabla_Alumnos)

def mostrar_notas_dataframe(dic):
    diccionario = {}
    for alumno_id, alumno in dic.items():
        diccionario[alumno_id] = {
            "Nombre": alumno.nombre,
